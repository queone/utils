...
- IE1: Consolidate the byte-commatizing helper (duplicated in `cmd/retotal` and `cmd/vtrim` as `addCommas`/`formatBytes`) into a shared `internal/` package and migrate both callers.
